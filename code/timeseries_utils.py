...
def save_dataset(df,lbl_itos,mean=None,std=None, model=None, data_seed=None, down_sample_rate=None, target_root="",filename_postfix="",protocol=4):
    if mean is None and std is None:
        mean, std = dataset_get_stats(df)
    target_root = Path(target_root)
    
    
    if model is not None and data_seed is not None and down_sample_rate is not None:
        target_root = target_root/str(model)/str(down_sample_rate)/str(data_seed)
    print("Saving to",target_root)
...
def load_dataset(target_root,filename_postfix="", df_mapped=True):
    target_root = Path(target_root)
    """if data_seed is not None and down_sample_rate > 0.0:
        if os.path.exists(target_root/down_sample_rate/data_seed/("df"+filename_postfix+".pkl")):
            target_root = target_root/down_sample_rate/data_seed
            loaded_dsr_data = True
    else:
        loaded_dsr_data = False"""
    if(df_mapped):
    ...
    mean = np.load(target_root/("mean"+filename_postfix+".npy"), allow_pickle=True)
    std = np.load(target_root/("std"+filename_postfix+".npy"), allow_pickle=True)
    return df, lbl_itos, mean, std #, loaded_dsr_data
...
class TimeseriesDatasetCrops(torch.utils.data.Dataset):
    ...
    def __init__(self, df, output_size, chunk_length, min_chunk_length, memmap_filename=None, num_classes=2, memmap_label_filename=None, npy_data=None, random_crop=True, data_folder=None, copies=0, col_data="data", col_lbl="label", cols_static=None, stride=None, start_idx=0, annotation=False, transforms=None, sample_items_per_record=1, fs_annotation_over_fs_data=1):
        """
