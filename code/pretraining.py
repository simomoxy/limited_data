...
if __name__ == '__main__':
    ...
    #    trainer.tune(model)
    if(hparams.s4 and hparams.resume!=""):
        model.load_s4_from_checkpoint(hparams.resume)
        lightning_state_dict = torch.load(hparams.resume)
        #trainer.fit_loop.epoch_loop.global_step = lightning_state_dict["global_step"]
        #trainer.fit_loop.epoch_progress.current.completed= lightning_state_dict["epoch"]

    if(hparams.epochs>0):
        if(MLFLOW_AVAILABLE):
            with mlflow.start_run(run_name=hparams.metadata) as run:
                for key, value in vars(hparams).items():
                    mlflow.log_param(key, " " if value=="" else value)
                trainer.fit(model,
        ckpt_path= None if (hparams.resume=="" or hparams.s4) else hparams.resume)
        else:
            trainer.fit(model)
