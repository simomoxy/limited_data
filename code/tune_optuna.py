...
def get_sampler(args, study_dir):
...
def cli_main():
    ...
    if args.objectives == 'multi':
        ...
        study = get_study_multi(args, study_dir, directions=["maximize", "minimize"], sampler=sampler, study_name=study_name, storage_name=storage)

        if len(study.get_trials()) < args.num_study_samples:
        ...
        sampler = get_sampler(args, study_dir)
        study = get_study_single(args, study_dir, direction="maximize", sampler=sampler, study_name=study_name, pruner=pruner, storage_name=storage)
