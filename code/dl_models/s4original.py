...
class HippoSSKernel(nn.Module):
    ...
    def forward(self, L=None, rate=None):
        #k, _ = self.kernel(rate=self.rate, L=L)
        # allowing to pass rate directly
        # MODIFIED
        k, _ = self.kernel(rate=self.rate if rate is None else rate, L=L)
        return k.float()
...
class S4(nn.Module):
    ...
    def __init__(
        ...
        transposed: choose backbone axis ordering of (B, L, H) or (B, H, L) [B=batch size, L=sequence length, H=hidden dimension] (B H L) if self.transposed else (B L H)
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        # MODIFIED
        k = self.kernel(L=L, rate=kwargs["rate"] if "rate" in kwargs.keys() else None) # (C H L) (B C H L)
