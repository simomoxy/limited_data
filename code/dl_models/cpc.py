...
class CPCModel(nn.Module):
    ...
    def __init__(self, input_channels, strides=[5,4,2,2,2], kss=[10,8,4,4,4], 
        ...
        if(s4):
            self.rnn = S4Model(
            d_input = self.encoder_output_dim if skip_encoder is False else input_channels,
            d_output = None,
            d_state = s4_d_state,
            l_max=s4_l_max,
            d_model = s4_d_model,
            ...
            dropout = s4_dropout,
            transposed_input = False, #behaves like LSTM with batch first
            bidirectional=False,
            pooling=False
            )
        else:
            self.rnn = nn.LSTM(self.encoder_output_dim if skip_encoder is False else input_channels,n_hidden,num_layers=n_layers,batch_first=True) if lstm is True else nn.GRU(self.encoder.output_dim if skip_encoder is False else input_channels,n_hidden,num_layers=n_layers,batch_first=True)
        
        if(num_classes is None): #pretraining
            if(mlp):# additional hidden layer as in simclr
                self.proj = nn.Sequential(nn.Linear(s4_d_model if s4 else n_hidden, n_hidden),nn.ReLU(inplace=True),nn.Linear(n_hidden, self.encoder_output_dim,bias=bias_proj))
            else:
                self.proj = nn.Linear(s4_d_model if s4 else n_hidden, self.encoder_output_dim,bias=bias_proj)
        else: #classifier
            ...
            #classifier
            if(self.s4):
                nf = 3*s4_d_model if concat_pooling else s4_d_model
            else:
            ...
            ps_head = [ps_head] if not isinstance(ps_head, Iterable) else ps_head 
            if len(ps_head)==1:
